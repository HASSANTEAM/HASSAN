...
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌁︙عدد اعضاء المجموعة اقل من ↫ *'..(DevAbs:get(HASSAN..'Abs:Num:Add:Bot') or 0)..'* عضو', 1, 'md')
return false
end
...
if text and (text:match("فروخ") or text:match("انكحك") or text:match("انكح") or text:match("قحبة") or text:match("قحبه") or text:match("قحاب") or text:match("مناويج") or text:match("زنى") or text:match("زنا") or text:match("زانية") or text:match("زاني") or text:match("منيوك") or text:match("انيك") or text:match("تسمك") or text:match("عيوشه") or text:match("عيوش") or text:match("عمر منيوج") or text:match("عمر ديوث") or text:match("رضاع الرجال") or text:match("رضاعة") or text:match("مرضعة") or text:match("مرضعه") or text:match("نزل بعائشه") or text:match("نزل بعائشة") or text:match("لعن الله هند") or text:match("اكلت الاكباد هند") or text:match("معاويه يشرب خمر") or text:match("معاوية يشرب عرك") or text:match("يزيد يشرب عرك") or text:match("لعن الله ابوبكر") or text:match("عمر منكوح")) then
...
local UserName = (dp.username_ or "GO_XXO")
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌁︙منشئ المجموعة ↫ ["..dp.first_name_.."](T.me/"..UserName..")", 1, "md")  
end,nil)   
end
end
end,nil)   
...
local UserName = (dp.username_ or "GO_XXO")
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌁︙تم رفع منشئ المجموعة ↫ ["..dp.first_name_.."](T.me/"..UserName..")", 1, "md") 
...
if text ==("كشففففف") and msg.reply_to_message_id_ ~= 0 and ChCheck(msg) or text ==("ايدي") and msg.reply_to_message_id_ ~= 0 and ChCheck(msg) then 
...
if text and text:match('^كشففف @(.*)') and ChCheck(msg) or text and text:match('^ايدي @(.*)') and ChCheck(msg) then 
local username = text:match('^كشففف @(.*)') or text:match('^ايدي @(.*)')
...
if text and text:match('كشففف (%d+)') and ChCheck(msg) or text and text:match('ايدي (%d+)') and ChCheck(msg) then 
local iduser = text:match('كشففف (%d+)') or text:match('ايدي (%d+)')  
...
if text == 'كشففف القيود' and tonumber(msg.reply_to_message_id_) > 0 and Admin(msg) and ChCheck(msg) then 
...
if text and text:match('^كشففف القيود @(.*)') and Admin(msg) and ChCheck(msg) then 
local username = text:match('^كشففف القيود @(.*)') 
...
if DevAbs:get(HASSAN..'Abs:Lock:Text'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","⌁︙تم فتح المنتدى تكلموا الآن بما يرضي الله جَزَاكُمُ الله خيرََا")  
DevAbs:del(HASSAN..'Abs:Lock:Text'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌁︙الدردشةبالفعل مفتوحه في المجموعة', 1, 'md')
end
end
...
if text == "كشففف البوتات" and ChCheck(msg) then 
...
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌁︙لمنشئ المجموعة فقط', 1, 'md')
else
...
if text == 'تفعيل كشففف الاعدادات' and ChCheck(msg) then 
local HASSANTEAM = '⌁︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌁︙تم تفعيل كشففف الاعدادات'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, HASSANTEAM, 14, string.len(msg.sender_user_id_))
DevAbs:set(HASSAN..'Abs:LockSettings'..msg.chat_id_,true)  
end
if text == 'تعطيل كشففف الاعدادات' and ChCheck(msg) then 
local HASSANTEAM = '⌁︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n⌁︙تم تعطيل كشففف الاعدادات'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, HASSANTEAM, 14, string.len(msg.sender_user_id_))
...
if text and text:match("^معلومات المجموعة$") and ChCheck(msg) or text and text:match("^عدد الاعضاء$") and ChCheck(msg) or text and text:match("^عدد الكروب$") and ChCheck(msg) or text and text:match("^عدد الادمنيه$") and ChCheck(msg) or text and text:match("^عدد المحظورين$") and ChCheck(msg) then
...
if text and text:match('^كشففف (-%d+)') then
local ChatId = text:match('كشففف (-%d+)') 
if not SudoBot(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌁︙للمطورين فقط', 1, 'md')
else
...
Dev_Abs(msg.chat_id_, msg.id_, 1, "⌁︙لم تتم اضافتي بها لاقوم بكشفففها", 1, "md")
end
end,nil)
end,nil)
end 
